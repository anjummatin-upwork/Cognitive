...
class GetYourReport extends Component {
  ...
  handleFileChange = (e) => {
    ...
    reader.onload = () => {
      ...
      this.state.list = this.state.fileContent
        ...
        .replace(/^.*#.*$/gm, "")
        .split(" ");
      //For Ancestory Genome DB
      if (this.state.list[1].split(/\s+/).slice(4, 5)[0] != "") {
        console.log("Anc DB");
        ...
        //For 23 you&me DB
        this.state.list.map((rnaMap) => {
          if (rnaMap.split(/\s+/).slice(3, 4)[0] != undefined) {
            this.state.dict.push({
              key: rnaMap.split(/\s+/).slice(0, 1),
              value:
                "(" +
                rnaMap.split(/\s+/).slice(3, 4)[0].slice(0, 1) +
                ", " +
                rnaMap.split(/\s+/).slice(3, 4)[0].slice(1, 2) +
                ")",
            });
          }
        });
  ...
  findGenoTypeDescription() {
    ...
    database
      .ref("Reports")
      .child(JSON.parse(localStorage.getItem("user")))
      .child(this.state.fileName.split(".").slice(0, -1).join("."))
      .get()
      .then((reports) => {
        if (reports.exists()) {
          // Alert
          Swal.fire({
            ...
            this.reportsPage(
              this.state.fileName.split(".").slice(0, -1).join(".")
